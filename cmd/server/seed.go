...
import (
	...
	"github.com/ctrl-research/getbud/internal/config"
	"github.com/ctrl-research/getbud/internal/store"
	"github.com/ctrl-research/getbud/migrations"
)

// runSeed implements `getbud seed`: idempotently create (or reset the
// password of) a local email/password user for development. Pair with
// GETBUD_LOCAL_AUTH=true to sign in with it.
func runSeed(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("seed", flag.ExitOnError)
	email := fs.String("email", "dev@getbud.local", "email of the local user")
	password := fs.String("password", "getbud-dev", "password of the local user")
