...
// GETBUD_OIDC_* one (Authentik, Keycloak, …). Both share the same
...
type ssoProvider struct {
	...
	// upsert resolves the token's identity to a getbud user.
...
func (p *ssoProvider) stateCookie() string    { return "getbud_oauth_state_" + p.key }
func (p *ssoProvider) verifierCookie() string { return "getbud_oauth_verifier_" + p.key }
...
func (s *Service) handleSSOCallback(p *ssoProvider) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if errors.Is(err, ErrEmailNotAllowed) {
			http.Error(w, "this getbud instance does not allow sign-ups for "+claims.Email, http.StatusForbidden)
			return
		}
		if err != nil {
