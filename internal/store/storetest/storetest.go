...
import (
	...
	"github.com/ctrl-research/getbud/migrations"
)

// Pool connects to GETBUD_TEST_DATABASE_URL, creating the database and
...
//
// Each calling package gets its own database (getbud_<pkg>_test): `go test
...
func Pool(t *testing.T) *pgxpool.Pool {
	...
	dbURL := os.Getenv("GETBUD_TEST_DATABASE_URL")
	if dbURL == "" {
		t.Skip("GETBUD_TEST_DATABASE_URL not set; skipping postgres-backed tests")
	}
...
// test package's directory name, e.g. getbud_test -> getbud_store_test.
func perPackageURL(t *testing.T, dbURL string) string {
	...
	if err != nil {
		t.Fatalf("parse GETBUD_TEST_DATABASE_URL: %v", err)
	}
...
func ensureDatabase(t *testing.T, ctx context.Context, dbURL string) {
	...
	if dbName == "" {
		t.Fatal("GETBUD_TEST_DATABASE_URL has no database name")
	}
